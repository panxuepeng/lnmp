...
./configure --prefix=$tengine_install_dir --user=$www_user --group=$www_user --with-http_stub_status_module --with-http_spdy_module --with-http_ssl_module --with-ipv6 --with-http_gzip_static_module --with-http_realip_module --with-http_flv_module --with-http_concat_module=shared --with-http_sysguard_module=shared $malloc_module
make && make install
...
sed -i "s@^user www www@user $www_user $www_user@" $tengine_install_dir/conf/nginx.conf
