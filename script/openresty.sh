...
openresty_install_dir=$install_dir/openresty/$openresty_version
...
if [ "$je_tc_malloc" == '1' ];then
    ...
    mkdir /tmp/tcmalloc
    chown -R ${www_user}.$www_user /tmp/tcmalloc
fi

[ ! -d "$openresty_install_dir" ] && mkdir -p $openresty_install_dir
./configure --prefix=$openresty_install_dir --user=$www_user --group=$www_user --with-http_stub_status_module --with-http_v2_module --with-http_ssl_module --with-ipv6 --with-http_gzip_static_module --with-http_realip_module --with-http_flv_module $malloc_module
make && make install
if [ -e "$openresty_install_dir/nginx/conf/nginx.conf" ];then
    ...
    echo "OpenResty install failed, Please Contact the author! " 
    kill -9 $$
fi
...
sed -i "s@^user www www@user $www_user $www_user@" $openresty_install_dir/nginx/conf/nginx.conf
